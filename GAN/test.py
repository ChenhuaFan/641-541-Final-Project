...
def test_own_image(G1, G2, path, out_path, size, img_transform):
    ...
    with torch.no_grad():
        ...
        log_metric('detected_shadow', shadow_removal_image, 0)
...
def main(parser):
    ...
    if parser.load is not None:
        ...
        G1_weights = torch.load('./checkpoints/ST-CGAN_G1_'+parser.load+'.pth', map_location=torch.device(device))
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./checkpoints/ST-CGAN_G2_'+parser.load+'.pth', map_location=torch.device(device))
